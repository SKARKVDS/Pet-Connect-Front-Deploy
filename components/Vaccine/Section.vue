<script setup lang="ts">
  ...
  const tableRef = ref();
  const table = ref<Table<Vaccination> | null>(null);
  const search = ref("");
  ...
  defineProps<{vaccinations: Vaccination[] }>()


  const columns: ColumnDef<Vaccination>[] = [
    { accessorKey: "vaccineName", header: "Vaccine name", enableHiding: false },
    {
      accessorKey: "dateVaccination",
      header: "Vaccination date",
      enableHiding: true,
      cell: ({ getValue }) => formatDateFromTimestamp(getValue()) // Utilisation de la méthode de formatage
    },
    {
      accessorKey: "dateBooster",
      header: "Booster date",
      enableHiding: true,
      cell: ({ row }) => {
        const needBooster = row.original.needBooster; // Accès à la propriété `needBooster`
        const dateBooster = row.original.dateBooster; // Accès à la propriété `dateBooster`
        return needBooster ? formatDateFromTimestamp(dateBooster) : "------";
      }    },
    {
      accessorKey: "actions",
      header: "",
      ...
      enableHiding: false,
      cell: ({ row }) => {
        return h(resolveComponent("VaccineItem"), { vaccination: row.original });
      }
    },
  ];
