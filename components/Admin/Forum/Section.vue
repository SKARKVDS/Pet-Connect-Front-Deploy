<script setup lang="ts">
  ...
  const tableRef = ref();
  const table = ref<Table<Forum> | null>(null);
  const search = ref("");

  const columns: ColumnDef<Forum>[] = [
    { accessorKey: "name", header: "Name", enableHiding: false },
    { accessorKey: "countTopics", header: "Topic Count", enableHiding: true },
    {
      accessorKey: "actions",
      header: "",
      enableSorting: false,
      enableHiding: false,
      cell: ({ row }) => {
        return h(resolveComponent("AdminForumItem"), { forum: row.original });
      }
    },
  ];

  defineProps<{forums: Forum[] }>()
</script>
