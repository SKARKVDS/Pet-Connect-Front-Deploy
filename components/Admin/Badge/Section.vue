<script setup lang="ts">
  ...
  const tableRef = ref();
  const table = ref<Table<Badge> | null>(null);
  const search = ref("");

  const columns: ColumnDef<Badge>[] = [
    { accessorKey: "name", header: "Name", enableHiding: false },
    { accessorKey: "numberServices", header: "Minimum Service Count", enableHiding: true },
    {
      accessorKey: "actions",
      header: "",
      enableSorting: false,
      enableHiding: false,
      cell: ({ row }) => {
        return h(resolveComponent("AdminBadgeItem"), { badge: row.original });
      }
    },
  ];
