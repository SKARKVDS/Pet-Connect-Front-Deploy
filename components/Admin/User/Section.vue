<script setup lang="ts">
  ...
  const tableRef = ref();
  const table = ref<Table<User> | null>(null);
  const search = ref("");

  const columns: ColumnDef<User>[] = [
    { accessorKey: "userName", header: "UserName", enableHiding: false },
    { accessorKey: "firstName", header: "First name", enableHiding: true },
    { accessorKey: "lastName", header: "Last name", enableHiding: true },
    { accessorKey: "email", header: "E-mail", enableHiding: true },
    { accessorKey: "phoneNumber", header: "Phone Number", enableHiding: true },
    { accessorKey: "countService", header: "Service count", enableHiding: true },
    {
      accessorKey: "actions",
      header: "",
      enableSorting: false,
      enableHiding: false,
      cell: ({ row }) => {
        return h(resolveComponent("AdminUserItem"), { user: row.original });
      }
    },
  ];
