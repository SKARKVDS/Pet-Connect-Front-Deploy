<script setup lang="ts">
  ...
  import type { ReservationRequest, ReservationProposal } from "~/services/reservation/types";

  defineProps<{reservationProposal: ReservationProposal}>()
  ...
  <div class=" p-6 bg-white shadow-md rounded-lg">
    ...
    <div class="space-y-4">
      ...
      <div class="flex items-center">
        ...
        <span class="text-gray-800">{{ reservationProposal.typeName }}</span>
      </div>

      <!-- Temps de Réservation -->
      <div class="flex items-center">
        <span class="font-medium text-gray-600 w-32">Start:</span>
        <span class="text-gray-800">{{ formatDateFromTimestamp(reservationProposal.startTime) }}</span>
      </div>
      <div class="flex items-center">
        <span class="font-medium text-gray-600 w-32">End:</span>
        <span class="text-gray-800">{{ formatDateFromTimestamp(reservationProposal.endTime) }}</span>
      </div>

      <!-- Nom de l'utilisateur -->
      <div class="flex items-center">
        <span class="font-medium text-gray-600 w-32">Name:</span>
        <span class="text-gray-800">{{ reservationProposal.userName }}</span>
      </div>

      <div class="flex items-center">
        <span class="font-medium text-gray-600 w-32">Phone number:</span>
        <span class="text-gray-800">{{ reservationProposal.phoneNumber }}</span>
      </div>

      <div class="flex items-center">
        <span class="font-medium text-gray-600 w-32">Email :</span>
        <span class="text-gray-800">{{ reservationProposal.email }}</span>
      </div>

      <!-- Adresse -->
      <div class="flex items-center">
        <span class="font-medium text-gray-600 w-32">Address:</span>
        <span class="text-gray-800">{{ reservationProposal.address }}</span>
      </div>
      ...
      <div class="flex items-center">
        ...
        <span class="text-gray-800">
          <ul>
            <li v-for="(pet, index) in reservationProposal.pets" :key="index" class="ml-2 list-disc">
              {{ pet }}
            </li>
          </ul>
        </span>
